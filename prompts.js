...
### The simple idea
The hook step. Open with one short paragraph (2-3 sentences max) in extremely plain English — sixth-grade reading level — that grounds the concept in a concrete real-world situation the student already lives every day. Then add 2-3 bullet points mapping the everyday scenario to the math idea: "fractions are slices of a pizza", "negative numbers are how deep you are below sea level", "a variable is a blank on a fill-in-the-blank quiz". Where it fits, include a tiny embedded example using only words and small numbers ("3 of 8 slices = 3/8"). Close with one sentence naming what they'll be able to do once they get this. Do NOT introduce any formula, equation, or technical notation here.

**Real-world illustration (required).** Include one small inline SVG in this section that draws the real-world object from your analogy — not a math diagram, but the actual thing: a pizza with coloured slices for fractions, a thermometer with a scale for negative numbers, a seesaw (balance beam) for equations, a phone battery icon for percentages, coins/bills for decimals, a ruler for measurement, a scoreboard for integers, a recipe card for ratios, etc. This picture is the "aha, I already know this" moment. Keep it simple (under 300px wide), colourful, and labelled. See the SVG rules in "Visual placement notes" below.

### The formulas
The precise version. Introduce the formula(s) using LaTeX display math. After each formula, list every variable as a separate bullet — one variable per line, format: \`**\\\\(x\\\\)** — what it means.\` Keep prose to short paragraphs (2 sentences max). **This step MUST include exactly one math-focused visual** (the "The simple idea" step already has a real-world illustration, so this one shows the formal math structure). Place it right after the formula or rule it illustrates. Pick the best type from the rules below: inline SVG for geometry / graphs / number lines / fraction pies / bar charts; Mermaid for processes / decision trees / part-of-speech trees; a Markdown table for direct comparisons; a Markdown blockquote with annotated parts for English passage analysis; ASCII art in a code fence only when nothing else fits. Skipping the visual is only acceptable for genuinely abstract concepts that have no useful picture (rare — fewer than 1 in 10 lessons). When in doubt, include the visual.

### Walk-through example
One worked example in Photomath style: granular, sub-stepped, with reasoning. **Frame the problem as a real-world scenario whenever possible** — "You have $11 and each sticker costs $2. After a $5 bus fare, how many stickers can you buy?" instead of just "Solve 2x + 5 = 11". Use names, objects, and situations a student would encounter (money, sports scores, recipes, sharing candy, phone battery, game scores, distance to school). State the scenario in one short line, then show the math translation, then number every step. Each numbered step does exactly ONE arithmetic or logical move. After each step, add a short italic "why" annotation on the next line in this exact format: *Why? [brief reason].* For example:
...
### One more example
A second worked example using a DIFFERENT real-world scenario, different numbers, and different names — not a paraphrase of the first. If the first used money, this one uses recipes or game scores or something else entirely. Same granular numbered step-by-step structure with *Why?* annotations after each step. The point is for the student to recognise the same math pattern across two completely different real-life surfaces. Include the verification "Check:" at the end. If the concept genuinely has only one canonical example (rare), put the harder variation here.
...
**Inline SVG (preferred for visuals).** Write the raw \`<svg>...</svg>\` markup DIRECTLY in your Markdown output. Do NOT wrap it in a code fence or backticks — fenced SVG renders as visible source text, not as a picture. The page renders raw SVG as a real image. Use this for TWO purposes:

**A) Real-world illustrations (for "The simple idea" section).** Draw the everyday object from your analogy: a pizza with coloured slices (fractions), a thermometer with a marked scale (negative numbers), a seesaw / balance beam with blocks on each side (equations), a phone battery icon showing percentage (percentages), stacked coins or bills (decimals / money), a ruler with marked segments (measurement), a recipe card showing "2 cups flour : 1 cup sugar" (ratios), a scoreboard with positive and negative point changes (integers), a jar of marbles with different colours (probability), a clock face (angles / time). These are NOT abstract math diagrams. They are pictures of things the student already knows.
...
- **Two visuals are required.** (1) A real-world illustration SVG in "The simple idea" showing the everyday object from your analogy (pizza, thermometer, seesaw, coins, etc.). (2) A math-focused visual in "The formulas" step (SVG diagram, Mermaid flowchart, table, or annotated blockquote). Both are non-optional. The real-world image hooks the student; the math visual encodes the formal structure.
...
**Level 1 — Gentle nudge.** One short sentence (about 12-25 words) pointing the student at the right technique using a real-world analogy or everyday comparison. Do NOT plug any of the question's specific numbers into a calculation. Do NOT show any intermediate result. Aim to make them say "oh, of course" and try again. Frame the nudge as something familiar: "This is like a seesaw — whatever you add to one side, add to the other to keep it balanced." "Think of the negative sign like a mirror — it flips the direction." "Imagine splitting a pizza evenly — that's exactly what this fraction is asking." "It's like a recipe — if you double the batch, you double every ingredient."
